...
export async function generateMetadata({
  ...
  if (!district) return createMetadata({ title: 'District Not Found' });
  ...
  const title = `${district.name} Government Commitments ${year}`;
  const description = `Track ${stats.total} government commitments affecting ${district.name} district (${district.nameNe}), ${province?.name || ''}, Nepal. ${stats.delivered} delivered, ${stats.inProgress} in progress.`;

  return createMetadata({ title, description, path: `/district/${slug}` });
}

/* ───────────────────────── JSON-LD ───────────────────────── */

function DistrictJsonLd({
  district,
  provinceName,
  commitments,
}: {
  district: { slug: string; name: string; nameNe: string; province: string };
  provinceName: string;
  ...
  const schema = {
    ...
    '@type': 'WebPage',
    name: `${district.name} District Government Commitments`,
    description: `Tracking ${stats.total} government commitments in ${district.name} district, ${provinceName}, Nepal.`,
    url: `${siteUrl}/district/${district.slug}`,
    about: {
      '@type': 'AdministrativeArea',
      name: `${district.name} District`,
      alternateName: district.nameNe,
      containedInPlace: {
        '@type': 'AdministrativeArea',
        name: provinceName,
    ...
    mainEntity: {
      '@type': 'ItemList',
      name: `${district.name} Commitments`,
...
export default async function DistrictPage({
  ...
  return (
    <>
      <DistrictJsonLd district={district} provinceName={province.name} commitments={commitments} />
        ...
        <div className="border-b border-gray-800 bg-gray-900">
          <div className="mx-auto max-w-4xl px-4 py-3">
            <nav className="flex items-center gap-1 text-sm text-gray-400">
              ...
              <ChevronRight className="h-3.5 w-3.5" />
              <span className="text-gray-100">{district.name}</span>
            </nav>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-4xl px-4 py-8">
          ...
          <header className="mb-8">
            ...
            <h1 className="mb-1 text-2xl font-bold text-white sm:text-3xl">
              {district.name} Government Commitments {year}
            </h1>
            <p className="mb-2 text-lg text-gray-400">{district.nameNe}</p>
            <p className="text-gray-300">
              <Link href={`/province/${province.slug}`} className="text-blue-400 hover:text-blue-300">
                {province.name}
              </Link>
              {' '}&middot; Capital: {province.capital}
            </p>
          </header>
          ...
          <div className="rounded-2xl bg-gradient-to-r from-indigo-600 to-blue-600 p-6 text-white">
            ...
            <p className="mb-4 text-sm text-indigo-100">
              See how government commitments impact {district.name} and the rest of {province.name}.
            </p>
          ...
          <div className="mt-6 border-t border-gray-700 pt-4">
            <p className="text-xs text-gray-500">
              {stats.total} commitments tracked for {district.name} district, {province.name}.{' '}
