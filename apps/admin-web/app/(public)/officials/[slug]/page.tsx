...
function getOfficialCommitments(official: OfficialProfile): GovernmentPromise[] {
  const pub = getPublicPromises();
  return pub.filter((p) => official.commitmentIds.includes(Number(p.id)));
}
...
export async function generateStaticParams() {
  return getAllOfficials().map((o) => ({ slug: o.slug }));
}
...
export async function generateMetadata({
  ...
  const official = getAllOfficials().find((o) => o.slug === slug);

  if (!official) return createMetadata({ title: 'Official Not Found' });

  const title = `${official.name} — Nepal Republic Accountability`;
  const description = `Track government commitments under ${official.name}. ${official.commitmentIds.length} commitments across ${official.ministries.length} ministries tracked by Nepal Republic.`;
...
function OfficialJsonLd({ official, commitments }: { official: OfficialProfile; commitments: GovernmentPromise[] }) {
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.nepalrepublic.org';
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'Person',
    name: official.name,
    url: `${siteUrl}/officials/${official.slug}`,
    jobTitle: official.name,
    worksFor: official.ministries.map((m) => ({
      '@type': 'GovernmentOrganization',
      name: m,
    })),
    knowsAbout: commitments.map((c) => c.title).slice(0, 10),
...
export default async function OfficialPage({
  ...
  const commitments = getOfficialCommitments(official);
  ...
  return (
    <>
      <OfficialJsonLd official={official} commitments={commitments} />
        ...
        <div className="border-b border-gray-800 bg-gray-900">
          <div className="mx-auto max-w-4xl px-4 py-3">
            <nav className="flex items-center gap-1 text-sm text-gray-400">
              ...
              <ChevronRight className="h-3.5 w-3.5" />
              <span className="text-gray-100">{official.name}</span>
            </nav>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-4xl px-4 py-8">
          ...
          <header className="mb-8">
            <div className="mb-4 flex items-center gap-4">
              <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-full bg-indigo-950 ring-2 ring-indigo-800">
                <User className="h-8 w-8 text-indigo-400" />
              </div>
              <div>
                <div className="mb-1 inline-flex items-center gap-2 rounded-full bg-indigo-950 px-3 py-1 text-xs font-medium text-indigo-300 ring-1 ring-indigo-800">
                  <Briefcase className="h-3.5 w-3.5" />
                  Government Official
                </div>
                <h1 className="text-2xl font-bold text-white sm:text-3xl">{official.name}</h1>
              </div>
            </div>
            {/* Ministries */}
            <div className="flex flex-wrap gap-2">
              {official.ministries.map((ministry) => (
                <span
                  key={ministry}
                  className="inline-flex items-center gap-1.5 rounded-full bg-gray-800 px-3 py-1 text-xs font-medium text-gray-300 ring-1 ring-gray-700"
                >
          ...
          <div className="mt-8 rounded-2xl bg-gradient-to-r from-indigo-600 to-blue-600 p-6 text-white">
            <h2 className="mb-2 text-lg font-bold">Track Accountability</h2>
            <p className="mb-4 text-sm text-indigo-100">
              See how {official.name} and other officials are performing on government commitments.
            </p>
          ...
          <div className="mt-6 border-t border-gray-800 pt-4">
            <p className="text-xs text-gray-500">
              ...
              Official associations derived from commitment ownership data.
            </p>
          </div>
        </div>
      </div>
