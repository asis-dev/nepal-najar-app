/**
 * Scraper for Ministry of Home Affairs — moha.gov.np
 * Security, governance, and administrative updates.
 */
import * as cheerio from 'cheerio';
import type { ScrapedArticle, SourceScraper } from '../types';

export const mohaGovScraper: SourceScraper = {
    ...
    for (const selector of selectors) {
      ...
      $(selector).each((_, el) => {
        ...
        if (headline && href && headline.length > 5 && !seen.has(href)) {
          seen.add(href);
          const fullUrl = href.startsWith('http') ? href : `https://moha.gov.np${href}`;
          articles.push({
