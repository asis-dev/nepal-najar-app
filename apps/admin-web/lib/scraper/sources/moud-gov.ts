/**
 * Scraper for Ministry of Urban Development — moud.gov.np
 * Urban planning, housing, and development projects.
 */
import * as cheerio from 'cheerio';
import type { ScrapedArticle, SourceScraper } from '../types';

export const moudGovScraper: SourceScraper = {
  name: 'Ministry of Urban Development',
  slug: 'moud-gov',
  url: 'https://moud.gov.np',
  type: 'government',
  language: 'ne',
    ...
    for (const selector of selectors) {
      ...
      $(selector).each((_, el) => {
        ...
        if (headline && href && headline.length > 5 && !seen.has(href)) {
          seen.add(href);
          const fullUrl = href.startsWith('http') ? href : `https://moud.gov.np${href}`;
          articles.push({
            source_name: 'Ministry of Urban Development',
            source_url: fullUrl,
