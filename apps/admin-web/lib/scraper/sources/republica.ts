/**
 * Scraper for MyRepublica — myrepublica.nagariknetwork.com
 * Major English-language Nepali news site.
 */
import * as cheerio from 'cheerio';
import type { ScrapedArticle, SourceScraper } from '../types';

export const republicaScraper: SourceScraper = {
  name: 'MyRepublica',
  slug: 'republica',
  ...
  async scrape(): Promise<ScrapedArticle[]> {
    ...
    const selectors = ['article', '.news-card', '.main-news', '.story', '.card'];
      ...
      $(selector).each((_, el) => {
        ...
        if (headline && href && headline.length > 10 && !seen.has(href)) {
          ...
          const fullUrl = href.startsWith('http')
            ? href
            : `https://myrepublica.nagariknetwork.com${href}`;
          articles.push({
    ...
    if (articles.length === 0) {
      $('a[href*="/news/"], a[href*="/economy/"], a[href*="/politics/"]').each((_, el) => {
