...
export function CorruptionComments({ caseSlug }: { caseSlug: string }) {
  const { isAuthenticated } = useAuth();
  const [newComment, setNewComment] = useState('');
  const queryClient = useQueryClient();

  const { data, isLoading } = useQuery<CommentsResponse>({
    queryKey: ['corruption-comments', caseSlug],
    queryFn: async () => {
      const res = await fetch(`/api/corruption/comments?slug=${caseSlug}`);
  ...
  const submitMutation = useMutation({
    mutationFn: async (content: string) => {
      const res = await fetch('/api/corruption/comments', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ slug: caseSlug, content }),
      });
      if (!res.ok) {
    ...
    onSuccess: () => {
      setNewComment('');
      queryClient.invalidateQueries({ queryKey: ['corruption-comments', caseSlug] });
    },
  });
  ...
  return (
    <div className="glass-card p-5">
      <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">
        ...
        Comments
        {comments.length > 0 && (
          <span className="text-[10px] px-2 py-0.5 rounded-full bg-primary-500/10 border border-primary-500/20 text-primary-400 font-medium">
            {comments.length}
          </span>
        )}
      </h3>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-6">
          ...
          <p className="text-xs text-gray-600 mt-1">Be the first to share your thoughts on this case</p>
        </div>
      )}
    </div>
  );
