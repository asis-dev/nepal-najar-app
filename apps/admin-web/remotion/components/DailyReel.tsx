...
/* ══════════════════════════════════════════════
   DAILY REEL (NEPALI) — 60s unified reel
   Everything in one punchy video.

   Timeline (1800 frames @ 30fps):
   0-3s    (0-90)     Hook — PM face + day counter
   3-6.5s  (90-195)   Stats flash — 3 big numbers
   6.5-16s (195-480)  Headlines — top 3 stories
   16-26s  (480-780)  Commitment tracker — bars + status
   26-36s  (780-1080) Minister spotlight
   36-50s  (1080-1500) Score reveal — gauge + grade
   50-60s  (1500-1800) CTA — engagement ending
   ══════════════════════════════════════════════ */
  ...
  return (
    <AbsoluteFill>
      ...
      <div style={{ position: 'absolute', top: 50, left: 0, right: 0, height: 750, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: 120, left: 40, right: 40 }}>
          <div style={{ background: 'rgba(0,0,0,0.7)', borderLeft: `4px solid ${RED}`, borderRadius: 8, padding: '12px 20px', display: 'inline-flex', flexDirection: 'column' }}>
            <span style={{ fontSize: 28, fontWeight: 900, color: WHITE }}>PM बालेन शाह</span>
            <span style={{ fontSize: 18, color: GOLD, fontWeight: 600 }}>प्रधानमन्त्री • RSP</span>
          </div>
        </div>
      </div>
      {/* Day counter + question */}
      <div style={{ position: 'absolute', bottom: 80, left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: '0 40px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, opacity: interpolate(frame, [3, 8], [0, 1], { extrapolateRight: 'clamp' }) }}>
          <span style={{ fontSize: 160, fontWeight: 900, color: WHITE, lineHeight: 1, fontVariantNumeric: 'tabular-nums', textShadow: `0 0 40px ${RED}60` }}>{displayDay}</span>
          <span style={{ fontSize: 56, fontWeight: 800, color: RED }}>दिन</span>
        </div>
        <div style={{ opacity: textOpacity, transform: `scale(${textScale})`, fontSize: 52, fontWeight: 900, color: WHITE, textAlign: 'center', lineHeight: 1.25, textShadow: '0 4px 20px rgba(0,0,0,0.8)' }}>
          सरकारले आज{'\n'}के गर्यो? 🤔
        </div>
      </div>
    </AbsoluteFill>
...
/* ══════════════════════════════════════════════
   STATS FLASH (3-6.5s): Big numbers
   ══════════════════════════════════════════════ */
      ...
      {stats.map((stat, i) => {
        ...
        return (
          <AbsoluteFill key={i} style={{ opacity: enter * exit, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: DARK }}>
            ...
            <div style={{ marginTop: 20, fontSize: 44, fontWeight: 800, color: stat.color, textAlign: 'center', lineHeight: 1.2 }}>
              {stat.label}
            </div>
          </AbsoluteFill>
        );
      ...
      {stories.map((story, i) => {
        ...
        return (
          <AbsoluteFill key={i} style={{ opacity: enter * exit, background: DARK, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '100px 50px 120px' }}>
            ...
            <div style={{ transform: `translateY(${slideY}px)`, fontSize: 54, fontWeight: 900, color: WHITE, lineHeight: 1.25, textShadow: '0 2px 10px rgba(0,0,0,0.5)' }}>
              {title}
            </div>
            {/* Summary if available */}
            {story.summaryNe && (
              <div style={{ marginTop: 20, fontSize: 28, fontWeight: 500, color: 'rgba(255,255,255,0.75)', lineHeight: 1.4, opacity: interpolate(f, [15, 25], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }) }}>
            ...
            <div style={{ marginTop: 24, opacity: interpolate(f, [15, 25], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }), display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontSize: 22, color: GRAY, fontWeight: 600 }}>{story.signalCount} स्रोतबाट पुष्टि</span>
            </div>
...
/* ══════════════════════════════════════════════
   COMMITMENT TRACKER (16-26s): Status + progress bars
   ══════════════════════════════════════════════ */
function CommitmentTracker({ data }: { data: DailyReelData }) {
  ...
  // Phase 1 (0-150 frames / 5s): Status breakdown — big donut + numbers
  // Phase 2 (150-300 frames / 5s): Top movers — progress bars
  ...
  return (
    <AbsoluteFill style={{ background: DARK }}>
      ...
      <Sequence from={0} durationInFrames={150}>
        <AbsoluteFill style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 40px', gap: 24 }}>
          ...
          <div style={{
            ...
            fontSize: 36, fontWeight: 900, color: GOLD, letterSpacing: 4, textAlign: 'center',
          }}>
            १०९ वचनबद्धता
          </div>
          <div style={{
            opacity: interpolate(frame, [5, 15], [0, 1], { extrapolateRight: 'clamp' }),
            fontSize: 24, fontWeight: 600, color: GRAY,
          }}>
            कहाँ पुग्यो?
          </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, marginTop: 20, justifyContent: 'center' }}>
            {statusItems.map((item, i) => {
              ...
              return (
                <div key={i} style={{
                  opacity: itemOpacity,
                  width: 460, background: `${item.color}10`,
                  ...
                  <div>
                    <div style={{ fontSize: 64, fontWeight: 900, color: item.color, lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
                      {displayCount}
                    </div>
                    <div style={{ fontSize: 24, fontWeight: 700, color: WHITE, opacity: 0.8 }}>
                      {item.label}
                    </div>
                  </div>
                  {/* Percentage */}
      ...
      {/* Phase 2: Top movers with progress bars */}
        ...
        {(() => {
          ...
          return (
            <AbsoluteFill style={{ display: 'flex', flexDirection: 'column', padding: '100px 50px 120px', gap: 16 }}>
              <div style={{
                opacity: interpolate(f2, [0, 10], [0, 1], { extrapolateRight: 'clamp' }),
                fontSize: 34, fontWeight: 900, color: GOLD, letterSpacing: 3, marginBottom: 10,
              }}>
                📊 प्रगतिमा रहेका
              </div>
                ...
                return (
                  <div key={i} style={{ opacity: barOpacity, marginBottom: 8 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      <span style={{ fontSize: 28, fontWeight: 700, color: WHITE, flex: 1 }}>
                        {mover.titleNe || mover.title}
                      </span>
                      <span style={{ fontSize: 32, fontWeight: 900, color, marginLeft: 16 }}>
                        {Math.round(barWidth)}%
                      </span>
                    </div>
                    {/* Progress bar */}
                    ...
                    <div style={{ fontSize: 20, fontWeight: 600, color: GRAY, marginTop: 4 }}>
                      {statusLabel(mover.status)}
                    </div>
                  </div>
                );
              })}
...
/* ══════════════════════════════════════════════
   MINISTER SPOTLIGHT (26-36s): Who's accountable
   ══════════════════════════════════════════════ */
    ...
    <AbsoluteFill style={{ background: DARK }}>
      ...
      <div style={{ position: 'absolute', top: 50, left: 0, right: 0, height: 650, overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 22, fontWeight: 900, color: WHITE, letterSpacing: 2 }}>
            🎯 जिम्मेवार को?
          </span>
        </div>
      </div>

      {/* Minister info below photo */}
      <div style={{
        position: 'absolute', top: 650, left: 0, right: 0, bottom: 56,
        display: 'flex', flexDirection: 'column', padding: '30px 50px', gap: 16,
      }}>
        {/* Name + role */}
        <div style={{
          opacity: interpolate(frame, [8, 18], [0, 1], { extrapolateRight: 'clamp' }),
        }}>
          <div style={{ fontSize: 52, fontWeight: 900, color: WHITE }}>{data.minister.nameNe}</div>
          <div style={{ fontSize: 26, fontWeight: 700, color: GOLD, marginTop: 4 }}>{data.minister.roleNe}</div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ background: `${RED_LIGHT}15`, border: `2px solid ${RED_LIGHT}40`, borderRadius: 16, padding: '16px 24px', textAlign: 'center', flex: 1 }}>
            ...
            <div style={{ fontSize: 20, color: GRAY, fontWeight: 600 }}>रोकिएको</div>
          </div>
          <div style={{ background: `${GOLD}15`, border: `2px solid ${GOLD}40`, borderRadius: 16, padding: '16px 24px', textAlign: 'center', flex: 1 }}>
            ...
            <div style={{ fontSize: 20, color: GRAY, fontWeight: 600 }}>पूरा</div>
          </div>
        </div>

        {/* Today's headline about PM */}
        {headline && (
          <div style={{
            opacity: interpolate(frame, [40, 55], [0, 1], { extrapolateRight: 'clamp' }),
            background: 'rgba(255,255,255,0.05)',
            borderLeft: `4px solid ${RED}`,
            borderRadius: 8, padding: '16px 24px', marginTop: 8,
          }}>
            <div style={{ fontSize: 20, fontWeight: 700, color: RED, marginBottom: 4 }}>आजको शीर्षक</div>
            <div style={{ fontSize: 28, fontWeight: 700, color: WHITE, lineHeight: 1.3 }}>
              {headline.slice(0, 80)}
            </div>
          </div>
        )}
...
/* ══════════════════════════════════════════════
   SCORE REVEAL (36-50s): Dramatic gauge
   ══════════════════════════════════════════════ */
function ScoreReveal({ data }: { data: DailyReelData }) {
  ...
  return (
    <AbsoluteFill style={{ background: DARK, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
      <div style={{ fontSize: 30, fontWeight: 800, color: GOLD, letterSpacing: 6, opacity: interpolate(frame, [0, 10], [0, 1], { extrapolateRight: 'clamp' }) }}>
        REPUBLIC SCORE
      </div>
      ...
      <div style={{ position: 'relative', width: 400, height: 400, marginTop: 10 }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ fontSize: 24, color: GRAY, fontWeight: 700, marginTop: 4 }}>
            {isEarly ? 'प्रारम्भिक चरण' : 'GRADE'}
          </div>
        </div>
      </div>
      ...
      <div style={{ opacity: interpolate(frame, [100, 120], [0, 1], { extrapolateRight: 'clamp' }), display: 'flex', gap: 30, marginTop: 16 }}>
        {data.commitmentsMoved.confirms > 0 && (
          <div style={{ background: `${GREEN}15`, border: `2px solid ${GREEN}50`, borderRadius: 16, padding: '16px 36px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 22, color: GRAY, fontWeight: 600 }}>प्रगतिमा</div>
          </div>
        )}
        {data.commitmentsMoved.contradicts > 0 && (
          <div style={{ background: `${RED_LIGHT}15`, border: `2px solid ${RED_LIGHT}50`, borderRadius: 16, padding: '16px 36px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 22, color: GRAY, fontWeight: 600 }}>चिन्ताजनक</div>
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        fontSize: 36, fontWeight: 800, color: WHITE, textAlign: 'center', lineHeight: 1.4, padding: '0 40px', marginTop: 10,
      }}>
  ...
  return (
    <AbsoluteFill style={{ background: DARK, opacity: interpolate(frame, [0, 8], [0, 1], { extrapolateRight: 'clamp' }), display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, padding: '80px 40px' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 48, fontWeight: 900, color: WHITE }}>नेपाल रिपब्लिक</div>
        <div style={{ fontSize: 24, color: GOLD, fontWeight: 700, marginTop: 4 }}>Nepal Republic</div>
      </div>
      <div style={{ opacity: interpolate(frame, [15, 28], [0, 1], { extrapolateRight: 'clamp' }), background: `${RED}15`, border: `2px solid ${RED}50`, borderRadius: 20, padding: '24px 36px', textAlign: 'center', marginTop: 10 }}>
        <div style={{ fontSize: 38, fontWeight: 800, color: WHITE, lineHeight: 1.3 }}>सरकारलाई कति अंक{'\n'}दिनुहुन्छ? 🗳️</div>
        <div style={{ fontSize: 22, color: GRAY, marginTop: 12 }}>कमेन्टमा भन्नुहोस् 👇</div>
      </div>
...
/* ══════════════════════════════════════════════
   MAIN COMPOSITION — 60s unified reel
   ══════════════════════════════════════════════ */
export const DailyReel: React.FC<{ data: DailyReelData }> = ({ data }) => {
  return (
    <AbsoluteFill style={{
      ...
      {/* Headlines: 6.5-16s (195-480) — 3 stories × 95 frames */}
      <Sequence from={195} durationInFrames={285}>
        <Headlines data={data} />
      </Sequence>

      {/* Commitment tracker: 16-26s (480-780) */}
      <Sequence from={480} durationInFrames={300}>
        <CommitmentTracker data={data} />
      </Sequence>

      {/* Minister spotlight: 26-36s (780-1080) */}
      <Sequence from={780} durationInFrames={300}>
        <MinisterSpotlight data={data} />
      </Sequence>

      {/* Score reveal: 36-50s (1080-1500) */}
